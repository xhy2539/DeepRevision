...
def load_quiz_prompts():
    try:
        quiz_prompt_path = get_abs_path(prompts_conf["quiz_prompt_path"])
    except KeyError as e:
        logger.error(f"[load_quiz_prompts]在yaml配置项中没有quiz_prompt_path配置项")
        raise e

    try:
        return open(quiz_prompt_path, "r", encoding="utf-8").read()
    except Exception as e:
        logger.error(f"[load_quiz_prompts]解析出题提示词出错，{str(e)}")
        raise e
